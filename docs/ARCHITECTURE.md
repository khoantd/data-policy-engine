...
# Enforcement / Ràng buộc (ADL)

```adl
DESCRIPTION ROS Policy architecture must enforce hexagonal boundaries
REQUIRES ArchUnit

DEFINE SYSTEM ROS Policy AS ros_policy
  DEFINE DOMAIN core AS core
    DEFINE COMPONENT core_engine AS core_engine

  DEFINE DOMAIN ports AS ports
    DEFINE COMPONENT api AS api
    DEFINE COMPONENT sdk AS sdk
    DEFINE COMPONENT scheduler AS scheduler

  DEFINE DOMAIN adapters AS adapters
    DEFINE COMPONENT inmemory_adapter AS inmemory_adapter
    DEFINE COMPONENT sqlalchemy_adapter AS sqlalchemy_adapter
    DEFINE COMPONENT redis_adapter AS redis_adapter
    DEFINE COMPONENT openguardrails_adapter AS openguardrails_adapter
    DEFINE COMPONENT fastapi_adapter AS fastapi_adapter
    DEFINE COMPONENT celery_adapter AS celery_adapter

-- Structural invariants ------------------------------------------------------
ASSERT(COMPONENTS are only CONTAINED WITHIN SYSTEM)

ASSERT(core_engine is CONTAINED WITHIN core)

-- Core independence: core must not depend on any port or adapter ----------------
ASSERT(core_engine has NO DEPENDENCY on api)
ASSERT(core_engine has NO DEPENDENCY on sdk)
ASSERT(core_engine has NO DEPENDENCY on scheduler)
ASSERT(core_engine has NO DEPENDENCY on inmemory_adapter)
ASSERT(core_engine has NO DEPENDENCY on sqlalchemy_adapter)
ASSERT(core_engine has NO DEPENDENCY on redis_adapter)
ASSERT(core_engine has NO DEPENDENCY on openguardrails_adapter)
ASSERT(core_engine has NO DEPENDENCY on fastapi_adapter)
ASSERT(core_engine has NO DEPENDENCY on celery_adapter)

-- Optional: ports should not depend on adapters (ensuring clean boundary) -------
ASSERT(api has NO DEPENDENCY on inmemory_adapter)
ASSERT(api has NO DEPENDENCY on sqlalchemy_adapter)
ASSERT(api has NO DEPENDENCY on redis_adapter)
ASSERT(api has NO DEPENDENCY on openguardrails_adapter)
ASSERT(api has NO DEPENDENCY on fastapi_adapter)
ASSERT(api has NO DEPENDENCY on celery_adapter)

ASSERT(sdk has NO DEPENDENCY on inmemory_adapter)
ASSERT(sdk has NO DEPENDENCY on sqlalchemy_adapter)
ASSERT(sdk has NO DEPENDENCY on redis_adapter)
ASSERT(sdk has NO DEPENDENCY on openguardrails_adapter)
ASSERT(sdk has NO DEPENDENCY on fastapi_adapter)
ASSERT(sdk has NO DEPENDENCY on celery_adapter)

ASSERT(scheduler has NO DEPENDENCY on inmemory_adapter)
ASSERT(scheduler has NO DEPENDENCY on sqlalchemy_adapter)
ASSERT(scheduler has NO DEPENDENCY on redis_adapter)
ASSERT(scheduler has NO DEPENDENCY on openguardrails_adapter)
ASSERT(scheduler has NO DEPENDENCY on fastapi_adapter)
ASSERT(scheduler has NO DEPENDENCY on celery_adapter)
```
