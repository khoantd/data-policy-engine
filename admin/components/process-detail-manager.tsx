...
import {
  deleteProcessAction,
  setProcessPoliciesAction,
  updateProcessAction,
} from "@/lib/actions";
import { processPlaygroundHref } from "@/lib/system-request-context";
import { ProcessResponse } from "@/lib/types";
import { Button } from "@/components/ui/button";
import { Input, Select, Textarea } from "@/components/ui/field";
import { ErrorAlert, EmptyState, TableWrap } from "@/components/ui/layout";

export function ProcessDetailManager({
  process,
  linkedPolicyIds,
  allPolicyOptions,
}: {
  ...
  const updateBound = updateProcessAction.bind(null, process.id);
  const linksBound = setProcessPoliciesAction.bind(null, process.id);
    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-2 rounded-md border border-border bg-muted/30 p-3">
        ...
        <p className="text-xs text-muted-fg">
          Open Evaluate or Scan with this process preselected. Linked policies
          appear as governance context only (no request{" "}
          <code className="font-mono text-[11px]">source</code> mapping).
        </p>
        <div className="flex flex-wrap items-center gap-2">
          <Link
            href={processPlaygroundHref("/evaluate", process.id)}
          ...
          <Link
            href={processPlaygroundHref("/classify", process.id)}
            className="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-border bg-surface px-2.5 py-1.5 text-sm font-medium text-foreground motion-safe:transition-colors motion-safe:duration-150 hover:bg-muted"
          >
      ...
      <form action={updateAction} className="grid gap-3 md:grid-cols-2">
        <Input label="Name" name="name" required defaultValue={process.name} />
        <Input
          label="Owner"
          name="owner"
          defaultValue={process.owner ?? ""}
        />
        <Select label="Status" name="status" defaultValue={process.status}>
        ...
        <Input
          label="Tags"
          name="tags"
          defaultValue={(process.tags || []).join(", ")}
        />
        <Textarea
          label="Description"
          name="description"
        ...
        <div className="md:col-span-2 flex flex-wrap items-center gap-3">
          ...
          <Button
            ...
            onClick={() => {
              if (
                !window.confirm(
                  `Delete process ${process.name}? This cannot be undone.`,
                )
              ) {
                return;
              }
              startTransition(async () => {
                await deleteProcessAction(process.id);
                router.refresh();
              });
            }}
          >
      ...
      <section className="flex flex-col gap-3">
        ...
        {linkedPolicyIds.length === 0 ? (
          <EmptyState message="No policies linked to this process." />
        ) : (
          <TableWrap>
